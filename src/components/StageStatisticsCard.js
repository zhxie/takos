      ...
      <Card hoverable bordered={false} bodyStyle={{ padding: 0 }} style={{ cursor: 'default' }}>
        <Descriptions bordered style={this.props.style} column={4}>
          ...
          {(() => {
            if (!this.props.stage.isSalmonRun) {
              ...
              if (StorageHelper.showSplatNetStats()) {
                reference = this.props.stage.record;
              } else {
                reference = this.props.stage.statistics;
              }
              if (reference !== null) {
                return reference.map(element => {
                  return (
                    <Descriptions.Item
          ...
          {(() => {
            if (this.props.stage.isSalmonRun) {
              if (!StorageHelper.showSplatNetStats()) {
                return this.props.stage.statistics.map((element, index) => {
                  return (
                    <Descriptions.Item
                      key={index}
                      label={
